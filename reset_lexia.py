...
def main():
    ...
    for wait in (0.5, 2.0, 4.0):
        time.sleep(wait)
        if alive():
            print("устройство отвечает - сброс шины не потребовался")
            return 0

    if not args.hard:
        # Сброс шины СОЗНАТЕЛЬНО не делается по умолчанию. Проверено за один вечер
        # трижды: на залипшем устройстве dev.reset() не поднимает его, а добивает -
        # Lexia уходит с шины целиком (пропадает из lsusb при воткнутом кабеле и
        # горящем индикаторе) и возвращается только переподключением разъёма руками.
        # Так что лучше честно сказать про разъём, чем сделать хуже.
        print("устройство залипло и само не оживает.")
        print("Переткни USB-конец. Если не поможет - сними питание с OBD на 5 с:")
        print("индикатор питается с машины, и по USB интерфейс не перезагружается.")
        print("(./reset_lexia.py --hard всё же попробует сброс шины, но обычно он вредит)")
        return 1
    ...
    time.sleep(2)
    print("устройство отвечает" if alive() else "по-прежнему молчит - переткни разъём")
    return 0
