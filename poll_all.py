...
def poll_ecu(lex, tx, rx, info, verbose=False, dead=None):
    ...
    for req, ps in sorted(rest.items()):
        ...
        if not payload:
            ...
            # НЕ помечаем мёртвым по одному пустому ответу. Пробовали - и сразу
            # отравили рабочий запрос: 21C08001 объявился молчащим через минуту
            # после того, как успешно им же и читали. Причина была в пустой
            # квитанции на первое чтение после входа, теперь она лечится повторной
            # выборкой в Lexia.read. Список молчащих остаётся, но заполняется
            # измерением, а не догадкой на ходу.
            silent += len(ps)
        if payload[0] == 0x7F:
            refused += len(ps)
            continue
