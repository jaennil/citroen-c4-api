...
def poll_ecu(lex, tx, rx, info, verbose=False, dead=None):
    ...
    for req, ps in sorted(rest.items()):
        try:
            ...
            raise
        if not payload:
            # Мёртвым по одному пустому ответу НЕ помечаем: пробовали и сразу
            # отравили рабочий запрос - 21C08001 объявился молчащим через минуту
            # после того, как им же успешно читали. Причина была в пустой квитанции
            # на первое чтение после входа, теперь она лечится повторной выборкой в
            # Lexia.read. Список молчащих остаётся, но заполняется измерением.
            #
            # continue тут ОБЯЗАТЕЛЕН. Без него управление проваливалось на
            # payload[0] и вылетал TypeError - посреди транзакции, оставляя
            # интерфейс недоделанным. Именно это и валило сбор через полторы минуты:
            # ошибка была не в устройстве, а здесь.
            silent += len(ps)
            continue
        if payload[0] == 0x7F:
            refused += len(ps)
