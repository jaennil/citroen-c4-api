...
THRESHOLDS = {
    ...
    "MP_TENSION_BATTERIE_AU_REPOS":
        "ВНИМАНИЕ: сам ряд похож на неверно отмасштабированный. Показывает "
        "16.1 В, что для покоящегося свинцового аккумулятора невозможно (предел "
        "около 13 В). Сырое значение 1610 при множителе из базы DiagBox 0.01. "
        "Если поделить на 128, выходит 12.58 В - как по учебнику, но подгонять "
        "множитель по одному красиво выглядящему числу я не стал. "
        "Проверяется мультиметром на клеммах после ночного простоя. "
        "Сами границы верны для напряжения покоя: 12.7 - полный заряд, "
        "12.4 - около половины, ниже 12.0 - глубокий разряд.",
