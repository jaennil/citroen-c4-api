...
* **Коды состояний** - `enums.py`: число -> подпись, только значения, подтверждённые
  ...
  лямбды, педали, реле вентилятора и т.д., 29 параметров). В базе DiagBox таблицы
  состояний есть, но в клоне от них остались номера строк без текста - гадать не стали.
  Такие панели рисуются как state-timeline. Состояния подушек (9/0) не расшифрованы:
  левая сторона 9, правая 0 при исправных цепях, смысл неясен.
...
**И SQL сразу закрыл спор про реле вентилятора.** Байтовое сканирование дало
"1 = Relais ouvert", то есть реле разомкнуто, и это противоречило нашим замерам.
Запрос показывает правду: `MP_ETAT_RELAIS_GMV` имеет ровно два состояния,
00 = Inactif и 01 = Actif. Единица это реле ВКЛЮЧЕНО, наша расшифровка по замерам
верна, а сканирование подобрало состояния чужого параметра. Ещё один довод к правилу
"замер это факт, каталог это гипотеза" - и заодно к тому, что реляционную базу надо
читать реляционно.

Открытая задача: у одной мнемоники бывает несколько наборов состояний, по одному на
семейство блоков (у `MP_ETAT_GMP` это 00 Initialisation, 01 Coupé ou calé,
02 Démarrage, 04 Tournant, 08 Arrêté, 10 En redémarrage - при том что на нашей машине
значение 2 устойчиво означает "работает"). Значит выборку надо ограничивать блоком
через BLOCK/ECUVER, иначе получится каша. Это следующий шаг.
