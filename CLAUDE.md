...
**Scheduling, since 2026-09-11: `schedule.py` is the plan.** A task is a block, a set of
parameter names (`None` = whole catalogue) and an interval; each tick the scheduler runs the
most overdue task that is due, with `MIN_GAP` (20 s) between any two excursions. The owner's
trip is 30 minutes, so intervals are sized for that: engine *hot* set (13 names that all live
in `21C08001` + `21CB8001`, so two requests: coolant, rpm, ECU voltage, both lambdas, fuel
trim, adsorber load and purge, injection time, fan setpoint and relay) every 90 s, engine full
every 300 s, ABS and GEP 180 s, BSM / stalk / cluster 300 s, airbag / parking / rain / display
/ panel 900 s. Simulated over 30 minutes with measured excursion durations (1.8-3.9 s): 68
excursions, engine hot 16, ABS and GEP 9 each, rare blocks 2 each, BSI stream idle 9 % of the
time. The unit passes only `--ecu-every 20`; `--ecus ADDR[:s]` is now a manual override for
...
## Расписание обслуживания с зонами и алертами (2026-09-07)

Таблица `maintenance` (одна строка на позицию: интервал в км и/или месяцах, когда и на каком
пробеге делали, заметка) - в SQLite, целиком досылается каждым `sync.py`. Дашборд, ряд
"Обслуживание" вверху: bar gauge "Ресурс, %" и таблица "что, когда, когда следующее".
Ресурс = большее из доли по пробегу (против текущего `MP_KILOMETRAGE_TOTAL` из BSI) и доли
по времени; **позиция без даты последней замены = 100 %**: неизвестно, значит пора. Зоны
80/100 %, те же пороги в двух правилах алертов в `homelab-infra` (`citroen-maint-soon`
warning, `citroen-maint-due` critical) - по одному экземпляру на позицию, с её названием.
...
## Досылка: по таймеру, по событию и живая (2026-09-11)

Три пути в один Postgres, все через `sync.py`, все под одним замком `/tmp/c4-sync.lock`:

* **таймер** `c4-sync.timer`, каждые 15 минут - страховка, если остальное не сработало;
* **по событию** - `ExecStopPost=sync-now.sh` в `c4-telemetry.service`: сбор закончился
  (Lexia выдернута, 90 с тишины, остановка службы) - хвост уезжает сразу. Хук уже стоял
  раньше и был убран, потому что вне дома SSH висел три попытки по таймауту; теперь
  `sync-now.sh` начинает с 5-секундной проверки достижимости и вне дома выходит сразу;
* **живая** `c4-sync-live.service` (`sync-live.sh`), стартует вместе со сбором через
  `Wants=`, живёт с ним через `BindsTo=`: один постоянный SSH-туннель с port-forward и
  `sync.py` каждые 10 с (фактически 10-18 с, прогон занимает 5-8 с). Дашборд с
  `refresh: 10s` показывает поездку с задержкой 15-30 с. Без интернета в машине ждёт и
  проверяет дом каждые 30 с. По SIGTERM досылает хвост и выходит. `sync-now.sh` при
  активном живом режиме уступает ему, иначе второй туннель убивает первый: на сервере
  перед port-forward делается `pkill -f "port-forward svc/postgres-rw"`.

Проверено 22:47: 40 с живого режима - четыре досылки по 190-800 значений, хвост 721
значение по TERM. Туннель за это время один раз переоткрылся сам.

## Локальная Grafana: прямой эфир без туннеля (2026-09-11)

`local/docker-compose.yml` поднимает на ноутбуке Postgres (127.0.0.1:5433, car/car),
Grafana (анонимный просмотр, алерты выключены, порт наружу не публикуется) и Caddy на
...
Данные туда льёт `sync-local.sh` через `sync.py --state ~/.config/c4-can/local.state`:
второй получатель ведёт СВОЙ водяной знак по id и не трогает флаг `synced`, который
принадлежит кластеру - иначе кластер потерял бы строки. Каждые 2 с без туннеля, задержка
от машины до графика - секунды при `refresh: 10s` (в локальной Grafana минимальный
интервал обновления снижен до 2 с). Служба `c4-sync-local` стартует и умирает вместе с
`c4-telemetry`. С телефона: `http://c4.local/d/citroen-c4`.
Первый запуск на пустой базе перегоняет весь car.db - минуты, один раз.

## Дашборд: состояния, единицы, дедупликация, алерты (2026-09-12)

По просьбе владельца всё, что меняется во времени, рисуется графиком (см. row()), а
следом закрыты пять из семи претензий аудита:

* **Коды состояний** - `enums.py`: число -> подпись, только значения, подтверждённые
  данными этой машины (разбор каждого перечисления против оборотов и скорости в тот же
  момент: ключ 0/1, ГМП 0/2, главное состояние BSI 105/150, сеть 0/5, блокировка 1/2,
  лямбды, педали, реле вентилятора и т.д., 29 параметров). Такие панели рисуются как
  state-timeline. С 13.09 поверх замеров подмешан каталог DiagBox, взятый по версии
  каждого блока (см. "Состояния параметров" ниже): подписано 284 параметра вместо 28,
  в том числе подушки - 9 это пиропатрон исправен, 0 это не сконфигурировано, так что
  прежняя запись "смысл неясен" снята.
* **Единицы** - `gunit()` в make_dashboard: поправки по имени `UNIT_OVERRIDE` там, где
  каталог врёт про саму единицу (педаль газа "V" при 390..2290 - это мВ; уровень топлива
  "litres" при 81 и баке 61 л - это проценты; давление хладагента "mbar" при 3.7 - бары;
  моменты двигателя "%" при -32..40 - не проценты), затем нормализация строки
  `UNIT_NORM` (V/Volt(s)/mv, Rpm/rpm, day(s), mOhms, мусор вроде "�S").
* **Правдоподобие** - проход по всем 205 параметрам с единицами против физических
  диапазонов дал 24 подозрительных; все либо поправлены выше, либо это счётчики с
  неверной единицей в каталоге. Один выброс: пробег при последнем сбросе ТО 1.857e6 км
  однократно - битый кадр, не исправлялся.
* **Дубли** - параметры ручного ряда "Другие блоки: главное" исключены из рядов своих
  блоков; идентификационные поля всех блоков (номера, версии ПО, серийники, 121 штука)
  собраны в одну свёрнутую таблицу "Идентификация блоков" вместо восьми панелей
  "Версия ПО". Скорость по ABS и по насосу ГУР подписаны с источником.
* **Мелочи** - точки на графиках редких блоков (`showPoints: auto`), список
  обозревателя с приставкой блока ("двигатель: ..."), и три правила алертов в
  `homelab-infra/monitoring/grafana/alerting.yaml`: `citroen-coolant-hot` (ОЖ > 105 °C
  3 мин), `citroen-voltage-low` (BSI < 12.8 В при оборотах > 500, 5 мин),
  `citroen-lamps` (лампа давления масла, уровня ОЖ, тормозная жидкость). Правила
  проверены `yaml.safe_load` до пуша, Grafana после синка без рестартов.

Не сделано из аудита: 228 графиков-констант остались (владелец просил всё графиками) и
нагрузка ~140 открытых панелей раз в 10 с на кластер.

## Общий алерт по красным зонам (2026-09-12)

Зоны всех параметров теперь живут только в `norms.py` (восемь BSI-зон переехали туда из
`make_dashboard.THRESHOLDS`). `norms.red_zones()` вырезает из каждой зоны красные отрезки
(их может быть два - цепи пиропатронов ниже 1.5 и выше 4.5 Ом), `sync.py` при каждой досылке
переписывает ими таблицу `norm_zone` в Postgres, а одно правило `citroen-red-zone` в
`homelab-infra` соединяет её с последним значением каждого параметра за 30 минут и
заводит отдельный экземпляр алерта на каждую строку. **Новая зона в norms.py = новый
алерт**, без правки правил. Ручные правила `citroen-lamps`, `citroen-oil-temp-*`,
`citroen-batt-low` удалены как дубли; остались обслуживание, `citroen-coolant-hot` (105 °C
- это жёлтая зона, ранний сигнал) и `citroen-voltage-low` (условие "при работающем
двигателе" зоной не выразить).

`alert=False` в записи нормы оставляет красную зону на графике, но не шлёт её в телеграм:
топливо ниже 5 %, средний расход, собственный счётчик ТО машины, обороты, насыщение
адсорбера. Красная зона это "посмотри", алерт это "действуй", и они не всегда совпадают.
Проверка перед включением: по последним значениям за всю историю ни один параметр в красной
зоне не сидит, то есть правило включилось молча. 39 красных отрезков на 29 параметров.

### Зоны двух типов (2026-09-12, вечер)

Владелец поправил критерий: зона нужна не только там, где есть норма работы, но и там,
где есть физический предел - за бортом не бывает 100 °C, времени впрыска 40 мс, углу
опережения 80°. Выход за такую границу значит мёртвый датчик или сломанную расшифровку,
и это тоже алерт. Поэтому в `norms.py` два блока: **нормы работы** (коррекция смеси ±10/±20 %,
опорные 5 В датчиков, дорожки педали газа, атмосферное и впускное давление, разрежение
усилителя тормозов, давление хладагента, испаритель, датчик уровня топлива) и
**правдоподобие** (наружная и впускная температуры, времена впрыска, опережение,
наполнение, расход воздуха, угол дросселя, скорость, обороты, скорость руля, мгновенный
расход, расчётный уровень топлива). Итого 86 зон, 85 красных отрезков с алертом на 54
параметрах. Пре-флайт по всей истории: в красное попал один замер из 4262 (питание ЭБУ
< 12 В на прокрутке стартером), двухминутная выдержка алерта его отсекает.

Чему зоны не ставили осознанно: моментам двигателя (единица неизвестна), ШИМ подсветки,
часам и дате, счётчикам импульсов колёс, обученным значениям упоров - у них нет ни нормы,
ни физического предела, который датчик мог бы нарушить. Давление масла с ЭБУ (0..1 кПа)
это сломанная расшифровка полузакрытой записи, зона его не спасёт.

## Коды неисправностей стали временным рядом (2026-09-12)

Служба теперь читает DTC сама: в `schedule.py` появился тип задачи `kind="dtc"` (двигатель
раз в 10 минут, насос ГУР / ABS / блок реле / BSI раз в полчаса), `drive.read_dtc()`
вызывает `dtc_read.read_block` в той же вылазке и пишет в базу.

**Ключевая деталь - нули.** Пишется не только найденный код со своим байтом статуса, но и
НОЛЬ для тех кодов, что у этого блока уже встречались, а сейчас в ответе их нет
(`storage.names_like`). Без нулей в Grafana получались редкие точки: не видно ни когда код
появился, ни когда ушёл, и положить его рядом с температурой или оборотами для поиска
совпадений нельзя. Проверено на заглушке: код исчезает из ответа - ряд уходит в 0.

На дашборде в ряду "Коды неисправностей" добавлена панель "История кодов" (state-timeline,
подписи статусов из `enums.ENUMS["DTC"]`: 0 нет, 1 активен, 0x28 сохранён и т.д.), таблица
текущих статусов осталась под ней.

**Коды читаются и сразу при подключении.** При старте службы у всех задач `due=0`, то есть
все просрочены, и планировщик берёт их по порядку списка раз в `MIN_GAP`. Поэтому порядок в
`SCHEDULE` важен ровно один раз - на старте: первым стоит горячий набор двигателя, следом
пять задач с кодами, так что через полторы минуты после втыкания Lexia прочитано и то, и
другое. Короткая поездка без чтения кодов невозможна.

Частоты после правки 12.09 (владелец: "если дешёвые, давай почаще"): коды двигателя раз в
5 минут, коды остальных блоков раз в 15. Место в расписании освобождено за счёт редких
блоков (подушки, парктроник, датчик дождя, дисплей, панель, дверь) - они переведены с 900 на
1800 с, там почти всё константы. Симуляция 30-минутной поездки: 72 вылазки, поток BSI
простаивает 9 % времени, коды двигателя снимаются 6 раз, коды прочих блоков по 2. Юнит службы не менялся, так что новый код подхватится
сам при следующем подключении Lexia - `install-automation.sh` не нужен.

## Образ DiagBox вскрыт: 370 описаний кодов вместо 54 (2026-09-13)

`mount-diagbox.sh` цепляет VMDK через qemu-nbd только на чтение (обычный loop не
берёт разреженный образ) и монтирует системный NTFS в `/mnt/diagbox`. Под sudo
`$HOME` это /root, поэтому путь к образу берётся из `getent passwd $SUDO_USER`.

**Что где лежит.** `AWRoot/dtrd/comm/data/DSD.FDB` (340 МБ) - определения параметров
и кодов, текст ВНУТРИ базы, по-французски, записями вида `<длина+2><длина>\x00<строка>`.
`GPC.FDB` (461 МБ) - те же коды, но описания ссылками `@P<номер>-POLUXDATA`.
`APP/LEXIA/DICO/citroen/dicoA??.zcl` - словари по языкам: заголовок `1B1B`, таблица
смещений с байта 108, дальше строки cp1251 с нулём в конце; RU 75 878 записей,
GB и FR по 108 739.

**Про "битые ссылки @P": вывод был неверный, см. раздел про GPC в конце файла.**
Здесь стояло, что ссылки не сходятся со словарём и GPC собран из другого обновления.
На деле нумерация в ссылках ведётся С ЕДИНИЦЫ, а читалась как индекс с нуля - отсюда
стабильный сдвиг на одну строку, дававший соседний, часто близкий по теме текст.

**Что извлечено.** `gen_dtc_dsd.py` -> `dtc_fr.py`: 370 описаний (193 P, 131 B, 30 U,
16 C). `dtc_tr.py` -> `dtc_names_ru.py`: перевод на русский по словарю терминов, а не
пофразно, потому что описания формульные; непереведённые слова видны через `--left` и
остаются французскими, а не выдумываются. `dtc_read.describe()` берёт источники по
убыванию точности: проверенные вручную KNOWN, 54 записи старого скана, 370 новых,
дальше честная отсылка к DiagBox.

Коды BSI вида B1003/B1137/B1228 через DSD не закрываются: там вместо описания
"Group_1". Закрыты они через GPC.FDB - см. раздел в конце файла.

**`gen_states.py`: 759 параметров с текстами состояний, и все требуют сверки.**
`--check` сравнивает их с `enums.py`, собранным по нашим замерам, и сверка сразу
поймала главное: DiagBox для `MP_ETAT_RELAIS_GMV` пишет "1 = Relais ouvert", то есть
реле разомкнуто. По нашим данным наоборот: при значении 1 задание вентилятора в
среднем 61.7 % и никогда не ноль, ОЖ в среднем 92.3 °C, при значении 0 задание 23.2 %
и ОЖ 72.2 °C. То есть 1 это вентилятор ВКЛЮЧЁН, наша расшифровка верна, а текст в базе
относится к другому семейству блоков. Та же картина у `MP_ETAT_GMP`: наши 0 и 2
коррелируют с оборотами, а в базе для этих значений совсем другие подписи. Правило
прежнее: каталог DiagBox это гипотеза, замер это факт.

Разовая переподпись уже накопленных кодов сделана прямо в SQLite (30 строк), и
`storage.param_id` теперь ВСЕГДА обновляет ярлык, если вызывающий передал его явно -
раньше ярлык дописывался только пустым, и новые описания не доезжали бы до дашборда.

### Словарь найден: POLUXDATA (2026-09-13, поздно)

Владелец возразил: раз DiagBox описания показывает, связь со словарём есть. Он был
прав, а я проверил не ту пару файлов. Ссылки вида `@P27831-POLUXDATA` называют не
просто номер, а ПРОСТРАНСТВО ИМЁН, и рядом с базами лежит
`AWRoot/dtrd/trans/POLUXDATAru_RU.DU8` - русский словарь на 55 620 строк.

Формат разобран и вынесен в `dico.py`: у всех трёх поколений словарей одна схема -
4 байта магии, три u32 (начало таблицы смещений, начало строк, число записей),
таблица смещений, строки подряд. Проверка целостности: `начало_таблицы + число*4 ==
начало_строк`. Различия: `.zcl` (LEXIA) - cp1251 и строки через `\x00`, `.DU8`
(POLUXDATA) - UTF-8 и границы по СЛЕДУЮЩЕМУ смещению, нулевого байта нет,
`THESAURU.dbz` - cp1251, 154 226 ремонтных фраз.

**Чего всё ещё не хватает: связи код -> номер.** Байтовое соседство не работает.
Для C1205 оно даёт правдоподобное "ошибка информации о величине угла поворота
рулевого колеса по CAN" (код живёт в насосе ГУР, звучит верно), а для P0116 все
пять кандидатов мимо: "включение правого указателя поворота", "Без тока", "Ошибка
сети CAN Комфорт". Значит связь в GPC.FDB реляционная, через ключи между таблицами
Firebird, и вытащить её сканированием байт нельзя.

Правильный путь - клиент Firebird и обычный SQL с JOIN. В системе его нет
(`isql-fb`, python-пакеты `fdb` и `firebird-driver` отсутствуют). Либо писать разбор
страниц Firebird самим: формат ODS, страницы данных, RLE-сжатие записей - это
отдельная работа, но без зависимостей.

Что уже работает и без этого: 370 описаний из inline-текста DSD.FDB (см. выше) и
поиск по словарю в обе стороны через `dico.py` (`dico.py POLUXDATA "температуры
охлаждающей"` находит номера по тексту).

### Базы DiagBox открыты по-настоящему: Firebird 2.5 в контейнере (2026-09-13)

Пакета `firebird` в репозиториях Manjaro нет (есть только `libfbclient`), в AUR лезть
не пришлось: базы открываются контейнером. `diagbox-sql.sh` поднимает
`jacobalberty/firebird:2.5-ss`, копирует DSD.FDB и GPC.FDB из образа в
`~/life/citroen/diagbox-db` (Firebird пишет в заголовок при подключении, а образ
смонтирован только на чтение) и выполняет запрос через isql. Пароль SYSDBA контейнер
генерирует сам и пишет в свой журнал, скрипт его оттуда и берёт.

Версия базы определяется по заголовочной странице: ODS 11 = Firebird 2.5, поэтому
именно 2.5, а не 3+.

**Схема DSD.FDB, что важно:** `PARAM` (PARID, PARSNAME - мнемоника, PARLNAME,
PARDESCRIPTION) и `STATES` (PARID как внешний ключ, STASNAME вида
`МНЕМОНИКА_ЗНАЧЕНИЕ`, STALNAME - текст состояния, STAVALUE - значение в hex). Плюс
BLOCK, ECUVER, SERVICE, SERVUNIT. То есть связь параметр-состояние-текст прямая, через
JOIN, и это ровно то, чего не хватало байтовому сканированию.

**Про реле вентилятора: запрос без привязки к блоку ничего не доказывает.** Здесь
раньше стояло, что SQL закрыл спор и `MP_ETAT_RELAIS_GMV` имеет ровно два состояния,
00 = Inactif и 01 = Actif. Это было получено выборкой по всей базе. С привязкой к
НАШЕЙ версии двигателя картина другая: у мнемоники 15 разных PARID, наш собственный
(56101 в `V46_32_B7`, 130390 в `V46_21_*`) не имеет текстов состояний ВООБЩЕ, девять
чужих говорят Inactif/Actif, а один (59001) - "Relais ferme / Relais ouvert", то есть
единица у него значит разомкнуто. Вывод про машину не меняется: замер однозначен
(при значении 1 задание вентилятора 61.7 % в среднем и никогда не ноль), но
подтверждения из базы у него нет - её просто нечем подтвердить.

## Состояния параметров: 1719 наборов, привязанных к версии блока (2026-09-13)

`gen_states_sql.py` -> `states_fr.py`, `states_tr.py` -> `states_ru.py`. На дашборде
расшифровано **284 параметра вместо 28**, и все 274 новых пришли из каталога.

**Охват берётся из `ecu_catalog`, а не угадывается.** У каждого блока там записан файл
DiagBox, из которого собран наш каталог (`BSI2010_B78_V1`, `V46_32_B7_DEGRADE`, ...), и
это же значение лежит в `ECUVER.ECUVEMNEMONAME` - ровно одна строка версии на блок. При
такой привязке спорных мнемоник не осталось НИ ОДНОЙ: все коллизии из "открытой задачи"
были следствием слишком широкой выборки, а не свойством базы.

Четыре вещи, каждая из которых давала неверный результат с первого захода:

* **Форма запроса решает всё: 11 секунд против 6 минут НА БЛОК.** Цепочка
  `ECUVER -> ... -> PARAM -> STATES` с `DISTINCT` раздувает промежуточный результат;
  вариант через `EXISTS` выполняет подзапрос на каждую строку PARAM (замерено: 6 мин
  23 с на один BSI). Работает разделение на два независимых среза - список PARID версии
  (14 с) и вся таблица состояний одной выгрузкой (0.5 с на 354 452 строки), - которые
  соединяются в питоне.
* **Подчёркивание в LIKE это шаблон "любой символ".** `'V46_32_B7%'` подбирает заодно
  `V46_32_B75C_*`, то есть другую платформу, а `'RBG_UDS%'` - случайные совпадения
  (семейство подушек в ECUVER зовётся иначе, и 202 "найденных" параметра были мусором).
  Поэтому сравнение на равенство, а не LIKE.
* **`STAVALUE` это не всегда шестнадцатеричное число.** У параметров с `PARENCODING`
  = `BMP` (bit map) значение записано ДВОИЧНОЙ строкой по ширине поля: "00", "01",
  "10", "11" это 0, 1, 2, 3. Решать надо на весь параметр сразу и по полю кодировки:
  правило "строка из нулей и единиц - значит двоичная" ломает перечисления, где
  значения 00, 01, 02, 04, 08, 10, 20 шестнадцатеричные, и "10" превращается в 2,
  затирая настоящее "заперто" подписью "передняя кабина отперта". Арбитр для ENUM,
  где встречается и то и другое, - маска поля из нашего каталога.
* **Диакритика потеряна в самом образе.** В части таблиц в поле лежит буквально байт
  `0x3F`: "Moteur arr?t?" вместо "Moteur arrêté". Проверено чтением столбца как
  `CHARACTER SET OCTETS` - колонка объявлена без кодировки, так что это не наша
  перекодировка. `states_tr.py` сопоставляет такие тексты по шаблону, где "?" это любой
  символ, и берёт перевод только при единственном совпадении.

**Что сверка дала по существу.** Из 28 наших расшифровок 13 нашлись в базе, 15 нет
(почти все - двоичные параметры двигателя, у которых в нашей версии текстов просто нет).

* База ПОДТВЕРЖДАЕТ замер: `MP_ETAT_VERROUILLAGE_VEHICULE` 1 = отперто, 2 = заперто;
  `MP_MOTIF_VERROUILLAGE_VEHICULE` 1 = радиобрелок, 8 = запирание на ходу;
  `MP_FONCTION_DE_REGULATION_A_AFFICHER` 2 = ограничитель скорости (это тот случай, где
  неверный разбор hex/двоичное сперва показал ложное противоречие);
  `GEP:MP_ETAT_MOTEUR_THERMIQUE_1_2` 3 = двигатель работает.
* База УТОЧНЯЕТ догадку: `MP_MOTIF_VERROUILLAGE_VEHICULE` 16 это кнопка центрального
  замка (у нас стояло "стоянка"), 32 - открытие двери внутренней ручкой ("открытие").
  Обе догадки убраны из `enums.py` в пользу каталога.
* База ОТВЕЧАЕТ на давний вопрос про подушки: 0 = не сконфигурировано, 9 = пиропатрон
  исправен, 10 = подрыв произведён, 12 = отключено, 14 = неисправно. Запись выше
  "смысл неясен" закрыта: левая сторона 9 это исправная цепь, правая 0 - место без
  пиропатрона.
* База ПРОТИВОРЕЧИТ замеру и проигрывает - список в `enums.CATALOG_WRONG`:
  `MP_ETAT_GMP` (каталог описывает сцепление автомата), `MP_INFO_GMP` (1 = "XX,X
  Volts"), `MP_POSITION_CLE_CONTACT` (знает только "нет/есть"), `MP_POSITION_LEVIER_
  VITESSE` (подставляет тексты разрешения пуска), `MP_CALCULATEUR_MOTEUR_VERROUILLE`
  (0 = "да", 1 = "нет", то есть перевёрнуто: замер даёт 1 только при нулевых оборотах).

**Проверка каталога данными машины, и она прошла.** Из 265 параметров, у которых есть и
состояния из каталога, и накопленные замеры, у 259 ВСЕ наблюдённые значения описаны
набором. То есть каталог ложится на эту машину, а не подписывает что попало. Отдельно
отброшены наборы, где вместо текстов лежат номера ("0" -> "2"), - подписать ими хуже,
чем не подписывать.

Приоритет в `enums.mappings()` жёсткий: каталог даёт основу, подтверждённые замером
подписи её перекрывают, а для `CATALOG_WRONG` каталог не подмешивается вовсе.


## Коды неисправностей: 1229 описаний из GPC.FDB (2026-09-13)

`gen_dtc_gpc.py` -> `dtc_gpc_ru.py`. Закрыты все коды этой машины кроме двух
(`B130E`, `B1507`): из 65 накопленных описание получили 63.

**Две ошибки прошлого захода, обе выглядели убедительно.**

* **Бралась строка чужого блока.** Код `0116` лежит в GPC 254 раза с 21 разной
  меткой - по строке на исполнение блока у каждой машины. С привязкой метка ровно
  одна. Цепочка: `VEHICULE` (платформа B7 = VEHID 28) -> `FAMILY` по адресу на шине
  (`FAMIDCANE`/`FAMIDCANR` это наши TX/RX) -> `ECU` по `ECUEDOFILENAME`, который
  называет исполнение (`INJ_V46_32_493_B7.xml`) -> `I_ECUDTC` -> `DTC`.
* **"Ссылки битые" - нет, нумерация с единицы.** Даже с правильной строкой P0116
  разрешался в "Не обнаружено ни одного датчика". Нужная строка на единицу ниже:
  `@P29613` -> индекс 29612 -> "Сигнал датчика температуры охлаждающей жидкости".
  Сдвиг на один давал соседа по словарю, а соседи там тематически близки - поэтому
  неверный ответ и выглядел правдоподобным. Проверено на восьми кодах с известным
  смыслом: P0130 -> "Сигнал верхнего кислородного датчика", C1205 -> "ошибка
  сигнала датчика угла поворота рулевого колеса", P2299 -> "сигнал датчика
  положения педали акселератора".

Ссылка бывает двух видов, `@P` и `@F`; словарь и нумерация у них одни. Разметка -
`@T`, `@\+`, `@\*N`; знаки препинания после `@T` это уже текст, а не разметка.
Словарь POLUXDATA русский, так что перевод не нужен вовсе.

**BSI зеркалит чужие коды.** В его собственном списке 458 кодов, и ни P0116, ни
C1205, ни U-кодов там нет (проверены варианты B7, B78 и EV - везде те же 458), а
читаем мы их именно ИЗ BSI. Поэтому `dtc_read._gpc()` ищет сначала у блока, откуда
код прочитан, а не найдя - у блоков, где такой код есть, и голосует. Сравнение идёт
без вводного слова: "Отсутствие связи с BSI" и "ошибка: отсутствие связи с BSI" это
один текст. Где расхождение настоящее (U0121, U1113, U1308 и ещё несколько),
показываются оба варианта через "/", а не выбирается наугад.

Порядок источников в `describe()`: `KNOWN` (проверено вручную на этой машине) ->
`dtc_gpc_ru` (привязано к исполнению блока) -> `dtc_names` (54 записи скана) ->
`dtc_names_ru` (370 из DSD, без привязки к блоку) -> честная отсылка к DiagBox.

`dico.py` копирует словарь из образа в `~/life/citroen/diagbox-db` при первом
открытии, так что монтировать образ ради словаря больше не нужно.

### Что расшифровка сразу показала про машину

Чтение 13.09; активны на тот момент 11 кодов, остальные - история в памяти.

* **Свет: ошибки включения обоих ближних (B1137/B1138, и в BSI, и в блоке реле),
  обоих передних габаритов (B114A/B114B), трёх указателей поворота (B1166/B1167/
  B1168), подсветки номера (B1163) и подсветки кнопок (B132B).** Это ровно то, чего
  следует ждать от ксенона в ближнем (зафиксирован выше как нештатный) и, судя по
  охвату, светодиодов в габаритах и поворотниках: BSI меряет ток лампы и считает
  нештатное потребление обрывом. То есть это следствие переделок, а не поломка
  проводки - и потому по этим кодам нельзя судить о реальном перегорании ламп.
* **Все четыре задних датчика парктроника (B1223, B1224, B1227, B1228)**, правые
  два - активные на момент чтения. Это уже не объясняется переделками.
* **Топливо: B1278, ошибка датчика уровня.** Рядом с давней путаницей в единицах
  уровня топлива на дашборде.
* **Кондиционер: B1301 (регулирование объёма компрессора) и B132A (защита контура
  низкого давления)**, оба и в BSI, и в блоке реле.
* **Двери и ключ: B1716, B171D (замок передней левой заперт, но читается открытым),
  B173D (опознание ключа), B1805 (стартер).**
* **Двигатель в зеркале BSI:** P0115/P0117 (ОЖ), P000A (фазорегулятор впуска),
  P0130 (верхняя лямбда), P0444 (клапан адсорбера), P0691/P0693 (обе скорости
  вентилятора) - все активные. При этом СОБСТВЕННЫЕ коды двигателя `V46_32:P0116`
  и `P2299` на последнем чтении уже НУЛЕВЫЕ, после замены ДТОЖ. Память BSI сама не
  чистится, поэтому её копии остаются и расходятся с двигателем.

### Два писателя кодов разошлись в именах (2026-09-14)

`drive.read_dtc` (служба) добавлял к имени тип отказа, а `dtc_read.py --sqlite`
(ручной прогон) - нет. Из-за этого один код с несколькими типами писался несколькими
замерами с ОДНИМ именем и ОДНИМ временем: B1228 пятью, B1137 тремя. Какой из них
возьмёт график - лотерея, а статусы у них разные, так что активный код мог
показаться просто сохранённым. Имя теперь строит общая `dtc_read.key()`, и ручной
путь заодно передаёт блок в `describe()`, то есть пользуется описаниями из GPC.

В накопленной истории это осталось: 49 замеров на 7 кодах. Смысл меняется ровно у
одного - `BSI2010:B1228` активен по одному из пяти типов; у остальных все типы со
статусом 8. Историю не переписывал: в ней честно лежат все типы, просто имя их не
различает. Следующее чтение запишет их раздельно.
