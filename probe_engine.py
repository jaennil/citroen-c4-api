...
РЕЗУЛЬТАТ ПРОВЕРКИ (2026-08-18): переключения НЕ происходит. С проигранной
преамбулой двигателя все контрольные DID отвергаются, а BSI продолжает отвечать.
Воспроизведения хвоста преамбулы недостаточно - канал задаётся историей всего
сеанса, а её мы не повторяем.

КАК ПРАВИЛЬНО ПРОВЕРЯТЬ. Годятся только DID, на которые BSI отвечает ОТКАЗОМ:
диапазоны 22E0xx, 22D7xx, 22FDxx (проверено: BSI даёт 7F 22 31, у двигателя там
752, 717 и 40 параметров). НЕ годятся 22D4xx и 22D5xx: у BSI там индексные DID,
они отвечают СПИСКАМИ других DID и выглядят как успех - на эти грабли я наступил
дважды, сначала приняв за подрулевой, потом за двигатель.
