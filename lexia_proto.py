...
class Lexia:
    ...
    def _collect(self, deadline: float = 3.0):
        ...
        while raw and len(raw) % 64 == 0:
            ...
            raw += more
            packets += 1
        # Подтверждать надо КАЖДЫЙ пакет, а не ответ целиком.
        #
        # Это и есть причина, по которой рушился возврат с KWP-блока на BSI, и три
        # версии до этого были неверны. Установлено дампом своего возврата
        # (capture-return.sh -> return.log) и сравнением с записью DiagBox:
        #
        #     мы:      после многопакетного ответа 064409 РОВНО ОДИН РАЗ, 23 из 23
        #     DiagBox: 064409 по нескольку подряд - два, три, четыре, 7 случаев из 25
        #
        # Что происходило без этого. Двигатель на 21 C0 80 01 отдаёт 252 байта -
        # четыре пакета. Мы забирали все четыре, подтверждали один раз, и у
        # устройства оставались неподтверждённые. После этого КАЖДАЯ следующая
        # команда получала отказ 15 40 09 E9, а на любой fetch приходил тот же
        # самый устаревший ответ - в дампе он повторяется восемь раз подряд. Отсюда
        # ровно 13 с до USBTimeoutError, одинаковые при любом маршруте возврата, и
        # Errno 5 после. И отсюда же то, что стоянка в одном блоке "работала":
        # устаревший ответ на месте прежнего запроса выглядит правдоподобно, а
        # ломались только команды переключения.
        for _ in range(max(1, packets)):
