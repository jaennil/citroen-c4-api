...
class Lexia:
    ...
    def transact(self, cmd: bytes, deadline: float = 3.0, dwell: float = 0.0,
        ...
        ПОВТОР ПРИ ОТКАЗЕ - страховка, а не норма. Раньше здесь было написано,
        что первую команду после таблицы устройство "отвергает всегда" и повтор
        это лечит. Неверно: DiagBox за 973 команды не получила ни одного отказа,
        потому что после ack на ответ таблицы выжидает 8-58 мс, а мы слали через
        2.4 мс. Отказ 15 40 09 02 - это "рано", а не "первая команда". Повтор
        оставлен: если отказ всё же случился, команду надо послать заново, но
        каждый такой случай - признак, что пауза перед ней мала (см. ecu.enter).
        """
        if expect is None:
            expect = echo_of(cmd)
        payload, raw = None, b""
    ...
    def _collect(self, deadline: float = 3.0):
        ...
        time.sleep(SETTLE)
        # Ответ устроен в ДВА уровня, и оба надо собирать.
        #
        # Уровень 1 - пакеты. Кадр идёт пакетами по 64 байта, ровно 64 значит
        # "продолжение следует". Без этого терялись все блочные чтения KWP: из 189
        # параметров двигателя читалось 21.
        #
        # Уровень 2 - кадры. Кадр вмещает не больше 256 байт (поле длины в байте 2
        # однобайтовое, 0xFC + 4). Ответ длиннее режется на кадры, и после ack за
        # неполным кадром устройство САМО, без poll и fetch, присылает следующий -
        # с 4-байтной шапкой 44 09 <len> <флаги> вместо обычной 24-байтной. Признак
        # "есть продолжение" - в байте 3 нет бита 0x40, точно как у фрагментов
        # команд: 0x80 первый, 0x41 последний, 0xC0 одиночный. На каждый кадр -
        # свой ack и свой контрольный байт в хвосте.
        #
        # Проверено по записи DiagBox без единого исключения: 855 кадров с 0xC0 -
        # за ack идёт команда, 4 с 0x80 - за ack идут данные. Мы второй уровень не
        # знали: после ack за 256-байтным кадром сразу слали следующую команду,
        # устройство отвечало отказом 15 40 09 E9 и на любой fetch отдавало хвост
        # незабранного ответа. Отсюда ровно 13 с до USBTimeoutError при возврате с
        # двигателя на BSI, одинаковые при любом маршруте, и "3 молчащих подряд" -
        # длинные ответы не молчали, их не удавалось разобрать.
        #
        # Правило "ack на каждый пакет" (91d4305) было неверным обобщением того же
        # наблюдения и ломало рукопожатие - см. историю. Верно: ack на каждый КАДР.
        def read_frame():
            fr = self._r()
            while fr and len(fr) % 64 == 0:
        ...
        time.sleep(POLL_SETTLE)
        while frames and len(frames[-1]) > 3 and not (frames[-1][3] & 0x40):
            nxt = read_frame()
            if not nxt:
                break
            frames.append(nxt)
            ...
            time.sleep(POLL_SETTLE)
        if len(frames) > 1:
            # шапка первого кадра 29 байт, продолжений - 4; контрольный байт у каждого
            payload = frames[0][29:-1] + b"".join(f[4:-1] for f in frames[1:])
            return payload, b"".join(frames)
    ...
    def transact_frames(self, frames, deadline: float = 5.0, dwell: float = 0.15,
        ...
        for f in frames[:-1]:
            self._w(f)
            # Квитанцию на КАЖДЫЙ фрагмент, и следующий - только после неё. Так
            # делает DiagBox: фрагмент 1 -> 06 40 09 через ~4 мс -> фрагмент 2 ->
            # 06 40 09 через ~76 мс -> готовность -> настоящий ответ 01 B4/01 CC.
            # Мы слали второй через 3.5 мс, до квитанции на первый, и устройство
            # отвечало иначе: одна квитанция на оба, затем ДВА пустых кадра
            # результата, а настоящий ответ - через ~490 мс, когда мы уже послали
            # 81/10 03 и получили 15 40 09 02. Так было на каждом входе в дампах
            # 04.09 20:55 и 07.09 21:09-21:14; у DiagBox за 76 таблиц - ни одной
            # пустой квитанции, ответ через 77-81 мс после последнего фрагмента.
            ack = self._r(timeout=500)
            if not ack or rejected(ack):
                log.warning(f"фрагмент команды: квитанция "
                            f"{ack.hex() if ack else 'не пришла'}")
        ...
        # Три быстрых цикла не хватало: два 78 подряд шли через 10 мс, а настоящий
        # ответ таблицы приезжал через ~300 мс. Ждём по времени, с паузой между
        # попытками, чтобы не долбить устройство опросами.
        t_pending = time.time()
        waited = 0
        while payload == b"\x78" and time.time() - t_pending < 0.6:
            time.sleep(0.02)
            payload, raw = self._collect(deadline)
            waited += 1
        if waited:
            log.info(f"статус 78: {waited} доп. цикл(а), {(time.time()-t_pending)*1000:.0f} мс -> "
