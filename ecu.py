...
# Пауза ПОСЛЕ положительного C1 у KWP-блока, перед первой командой в него. Измерено
# по двум нашим дампам (20:55 04.09 и 21:09 07.09): второй вход в двигатель получал
# c1d08f, мы через 2.5 мс слали кадр fe - и устройство глохло навсегда, ни одной
# квитанции больше. Первый вход с той же паузой проходил. DiagBox после c1 шлёт
# кадр fe только через 312-391 мс (а 21 fe / 21 80 - через 1-26 мс, но их у нас нет).
SETTLE_AFTER_KWP_ENTRY = 0.4
...
def enter(lex, tx, rx, verbose=False, _hop=False):
    ...
    for attempt in range(1, 4):
        ...
        if last and last[0] == expect:
            if expect == 0xC1:
                time.sleep(SETTLE_AFTER_KWP_ENTRY)
            break
