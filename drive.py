...
def main():
    ...
    # ПО УМОЛЧАНИЮ ПУСТО. Вылазка в чужой блок пока подвешивает интерфейс: после
    # неудачного снимка двигателя устройство отвечает USBError [Errno 5] на всё, и
    # служба уходит в бесконечный цикл переподключений, не собирая даже BSI. То есть
    # непочиненная вылазка ломает основную работу, а этого допускать нельзя.
    # Включать явно, для опытов: --ecus 6A8,6AD,747,75F,742,75D,730,765,77B,6B5
        ...
        if extra and t0 - last_ecu >= args.ecu_every and not os.path.exists(PAUSE_FLAG):
            ...
            try:
                ...
                log.warning(f"снимок {info['ru']} не удался ({type(e).__name__}: {e}) - "
                            f"больше вылазок в этом сеансе не будет")
                extra = []
